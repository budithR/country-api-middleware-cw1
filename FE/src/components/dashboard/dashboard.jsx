...
import { getAuthToken, getApiKeys, revokeApiKey, generateApiKey, getAllApiKeys } from '../../services/api'
...
const Dashboard = () => {
  ...
  const fetchApiKeys = async () => {
    try {
      ...
      const keys = isAdmin ? await getAllApiKeys() : await getApiKeys();
...
//   }, [navigate])
  ...
  const handleRevoke = async (keyId) => {
    try {
      ...
      setMessage({
        isDisplay: true,
        message: 'Key got revoked successfully',
        type: 'success',
      })

      // Hide after 2 seconds
      setTimeout(() => {
        setMessage(prev => ({ ...prev, isDisplay: false }))
      }, 2000)
    } catch (err) {
      setError(err.message)
    }
  ...
  const handleGenerate = async () => {
    ...
    //   setIsGenerating(true)
      ...
      setMessage({
        isDisplay: true,
        message: 'Key is generated successfully',
        type: 'success',
      })
        ...
        {apiKeys.length === 0 ? (
          ...
          <table className="api-keys-table">
            ...
            <tbody>
              {apiKeys.map((key) => (
                <tr key={key.id}>
                  {isAdmin && <td>{key.User?.username || 'Unknown'}</td>}
                  <td>{key.key}</td>
                  <td>
                  ...
                  <td>
                    {key.isActive && (
                      <button className="revoke" onClick={() => handleRevoke(key.id)}>
                        Revoke
                      </button>
                    )}
                  </td>
